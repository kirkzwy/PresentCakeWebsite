$(function(){
  ...
  // 存储默认数据
  var valDefault = [];
  $('input.signup-input').$.each(function(i, ele) {
  ...
  $('#nickname').focus(function(){
    ...
    $(this).removeClass('input-er');
	  ...
	  // function: reg_email.test()
	}else if( reg_email.test( $(this).val() ) ){
	  $(this).addClass('input-er');
	  $(this).after(username_50);
	}else{
	  var regData = _regData;
	  regData.name	= ($('#nickname').val() == valDefault[1]) ? '' : $('#nickname').val();
	  regData.pwd		= ($('#sign_up_password').val() == valDefault[2]) ? '' : $('#sign_up_password').val();
	  regData.email	= ($('#sign_up_email').val() == valDefault[0]) ? '' : $('#sign_up_email').val();
  ...
  $("#signupSubmit").click(function(){
		...
		var regData = _regData;
		regData.name	= ($('#nickname').val() == valDefault[0]) ? '' : $('#nickname').val();
		regData.pwd		= ($('#sign_up_password').val() == valDefault[1]) ? '' : $('#sign_up_password').val();
		regData.email	= ($('#sign_up_email').val() == valDefault[3]) ? '' : $('#sign_up_email').val();
		regData.act		= 'reg';
		//regData._b		= IF.CLIENT.build;
		//regData._s		= IF.ME.sid;
		$.ajax({
